    ...
    <>
      ...
      <div className="fixed top-0 right-0 bottom-0 w-full max-w-md bg-[#111] border-l border-[#222] z-[301] flex flex-col cart-drawer">
        ...
        <div className="flex items-center justify-between p-6 border-b border-[#222]">
          ...
          <button
            onClick={() => setIsOpen(false)}
            className="w-10 h-10 rounded-full border border-[#333] flex items-center justify-center text-gray-400 hover:text-white hover:border-white/20 transition-all"
          >
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
          {items.length === 0 ? (
            ...
            items.map((item) => (
              <div
                key={item.product.id}
                className="flex gap-4 bg-[#161616] border border-[#222] rounded-xl p-3 group"
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="text-sm font-medium text-gray-200 line-clamp-2 leading-snug mb-1">
                    {item.product.name}
                  </h3>
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    <button
                      ...
                      className="w-7 h-7 rounded-md bg-[#222] text-gray-400 flex items-center justify-center hover:bg-[#333] hover:text-white transition-colors text-sm"
                    >
                      -
                    </button>
                    ...
                    <button
                      ...
                      className="ml-auto w-7 h-7 rounded-md text-gray-600 flex items-center justify-center hover:bg-red-500/10 hover:text-red-400 transition-colors"
                    >
        ...
        {items.length > 0 && (
          <div className="border-t border-[#222] p-6 space-y-4">
            ...
            <button
              onClick={() => setIsOpen(false)}
              className="block w-full text-center text-gray-500 text-sm hover:text-white transition-colors py-2"
            >
              Continuar Comprando
            </button>
          </div>
